...
let hour;
let message;
hour < 12 ?  message = "Good morning" : message = "Good afternoon";
...
for(const key in students){
    console.log(`${students[key].name} Scored ${students[key].score}`);
...
const arrayTwo = students.map((student) => {
    if(student.score >=80){
        student.grade = 'A'
    }
...
const index =students.findIndex((student)=> student.name == 'Sneha')
console.log(index);
...
console.log(students.some((student)=>student.score >90))
console.log(students.every((student)=>student.passed == true))
...
console.log(students.reduce((a, student)=>{return a+student.score},0))
console.log(((students.reduce((a, student)=>{return a+student.score},0))/students.length).toFixed(2))
